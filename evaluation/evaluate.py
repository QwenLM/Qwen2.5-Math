...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    score_mat = []
    for sample in samples:
        if not sample['pred']:  # Skip samples with empty 'pred' list
            continue
    ...
    if not score_mat:  # Handle case where score_mat might be empty
        col_means = np.array([])
        mean_score = []
    else:
    ...
    result_json = {
        ...
        "empty_samples": len([s for s in samples if not s['pred']]),
        "acc": mean_score[0] if mean_score else 0
    }
    ...
    if "type" in samples[0]:
        ...
        for sample in samples:
            ...
            if sample['pred']:  # Add only if 'pred' is not empty
